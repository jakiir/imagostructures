...
function svc_inline_social_popup(){
		...
		<div class="svc_vimeo_popup">
			<div>
				...
				<div class="vimeo-panel-details">
					...
					<div class="vimeo_description"><?php echo $snippet->description;?></div>
				</div>
	...
	if($_GET['network'] == 'youtube'){?>
		...
		<div class="svc_youtube_popup">
			<div>
				...
				<div class="you-panel-details">
					<div class="you_publish_date">Published on <?php echo date('M d,Y',strtotime($snippet->publishedAt));?></div>
					<div class="you_description"><?php echo $snippet->description;?></div>
				</div>
	...
	if($_GET['network'] == 'instagram'){
		...
		$feed_url = 'https://www.instagram.com/p/'.$media->code.'/';
		//$response = wp_remote_get( $url );?>
			...
			<div>
				<div class="vcyt-col-md-7 vcyt-col-sm-6" style="line-height:0">
					...
					<?php }else{?>
					<a href="<?php echo $feed_url;?>" target="_blank"><img src="<?php echo $media->display_src?>" class="insta_popup_img"/></a>
					<?php }?>
				</div>
				...
				<div class="insta_popup_content">
					...
					<div class="insta_like_date">
						<div class="insta_likes"><i class="fa fa-heart-o"></i>&nbsp;<?php echo insta_like_counter($media->likes->count);?>&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-comments-o"></i>&nbsp;<?php echo insta_like_counter($media->comments->count);?></div>
						<div class="insta_date"><?php echo time_elapsed_string($media->date);?></div>
					</div>
					<div class="insta_caption">
						<?php echo $media->caption;?>
					</div>
					</div>
					<?php $comm_count = count($media->comments->nodes);
					if($comm_count > 0){?>
					<div class="insta_comments">
						<?php $comments = $media->comments->nodes;?>
						<span>Comments</span>
						<div class="insta_comments_inner">
						<?php for($i=0;$i<$comm_count;$i++){?>
						<p><a href="https://www.instagram.com/<?php echo $comments[$i]->user->username;?>" target="_blank"><?php echo $comments[$i]->user->username;?></a> <?php echo $comments[$i]->text;?></p>
						<?php }?>
						</div>
					</div>
					<?php }?>
