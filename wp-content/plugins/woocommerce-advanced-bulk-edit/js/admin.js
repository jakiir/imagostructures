...
//          posY = $(this).position().top;
	  ...
	  var selecthtml = '<select id="selectmapfield">';
		...
		{
			var col = _idmap[i];
			if(col.type !== 'int' && col.type !== 'float2' && col.type !== 'float3'  && col.type !== 'text'  && col.type !== 'multitext' && (col.field !== 'product_tag' && col.field !== 'product_cat'))
				continue;
...
//									instext+= '<br/><select data-id="attribute_'+attrslug+'">';
								...
								if(attrs[attrslug] === undefined)
									attrs[attrslug] = {};
//								instext+=	'<option value="">Any '+attr.name+'</option>';
//								 attrs[attrslug]['***thename***'] = attr.name;
...
//							instext+= '<br/><select data-id="'+col.field+'">';
						if(attrs[col.field] === undefined)
							attrs[col.field] = {};
//						instext+=	'<option value="">Any '+col.name.replace("(attr) ","")+'</option>';
// 						attrs[col.field]['***thename***'] = col.name.replace("(attr) ","");
			...
			for(var attr in attrs)
			{
				...
				}
				{
					if(attrib['getnext'] === true)
					{
						...
						{
//							if(attcounter > 0)
//								attrarr[attcounter - 1]['getnext'] = true;
							attrib['counter'] = 0;
						}
...
//		$('#bulkvarsdialog .editorcats input').attr('disabled','disabled');
	...
	function CreateVariations(pbatch)
	{
		...
		jQuery.ajax({
		     ...
		     success: function(response) {
					...
					DisableAllControls(false);
					$('.showajax').remove();
					if(response.hasmore !== undefined && response.hasmore === 1)
					{
...
//		$("#addok").button("disable");
	...
	var _idmap = [
		...
		{id:'post_content',field:'post_content',name:'P. Description',tooltip:'Product Description',width: 170,wordpress:true,scope:SCOPE.PRODALL,type:"multitext"},
		{id:'post_excerpt',field:'post_excerpt',name:'P. Excerpt',tooltip:'Product Short Description',width: 170,wordpress:true,scope:SCOPE.PRODALL,type:"multitext"},
...
//				}
			...
			if(params[field + 'action'] === "new" && selitem.post_type == 'product_variation')
			{
				curitem.name = params[field + 'name'];
			...
			if(params[field + 'action'] === "removevalue" && hasit)
			{
				if(selitem.post_type == 'product')
				{
					...
					var isremoved = false;
				     ...
				     {
					 	var value = $.trim(values[i]);
					 	for(var i = j; j < curvalues.length; j++)
				     	{
...
//						if(reptext != "")
		...
		if(col.type === 'int')
		{
			...
			if(params[field + 'mappedto'] !== undefined && action === "new")
			{
				...
				if(selitem[mappedto] !== undefined && selitem[mappedto] !== null)
				{
					bulkvalue = parseInt(selitem[mappedto]);
				}
			}
			if(isNaN(bulkvalue))
				return;
...
//			}
	...
	function GenerateGroupedItems(bgenattrs)
	{		
...
//			}
	...
	function SaveChanges(control,load,gotopage,pdata,pcolumns,pbatch)
	{
...
//										initem._product_permalink = selitem._product_permalink;
					...
					if(response.hasmore !== undefined && !load  && response.hasmore === 1)
					{
						if(response.savingbatch !== undefined)
						{
							$('#bulkeditinfo').text("");
						...
						SaveChanges("savechanges",false,0,ajaxarr.data,ajaxarr.columns,true);
						return;
					}
					$('#bulkedittext').text(W3Ex._translate_strings["trans_selected_text"] + ":");
					RefreshSelected();
...
//				}
	...
	$('#seldupproducts').click(function()
	{
		...
		for(var irow=0; irow < _data.length; irow++)
		{
			...
			if(selitem.post_type === 'product_variation')
				continue;
			if(obj[selitem.post_title] !== undefined)
			{
				if(obj[selitem.post_title] !== "-1")
				{
					selectedRows.push(obj[selitem.post_title]);
					obj[selitem.post_title] = "-1";
				}
				selectedRows.push(irow);
			}else
				obj[selitem.post_title] = irow;
		}
		obj = {};
		_grid.setSelectedRows(selectedRows);
		$("#selectdialog").dialog('close');	
