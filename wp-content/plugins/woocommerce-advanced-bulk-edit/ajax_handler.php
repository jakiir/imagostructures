...
///////////////////////////////////////
		if($bgettotalnumber)
		{
			$query = "INSERT INTO {$temptable} (
				...
				WHERE {$posts}.post_type='product'{$titlelike}{$desclike}{$shortdesclike} AND {$posts}.post_status IN ({$posttypesearch}) {$wherenotin} GROUP BY {$posts}.ID
				{$orderby})";
		}else
		{
			$query = "INSERT INTO {$temptable} (
				...
				FROM {$posts}
				{$catsquery}{$pricequery}{$salequery}{$skuquery}{$custommetasearch}
				WHERE {$posts}.post_type='product'{$titlelike}{$desclike}{$shortdesclike} AND {$posts}.post_status IN ({$posttypesearch}) {$wherenotin} {$idquery} GROUP BY {$posts}.ID {$orderby}{$limitquery})";
		}
		if($catsquery === '')
		{//let's get products without product_type'
			if($bgettotalnumber)
			{
				$query = "INSERT INTO {$temptable} (
					...
					WHERE ({$posts}.post_type='product'{$titlelike}{$desclike}{$shortdesclike} {$wherenotin} AND {$posts}.post_status IN ({$posttypesearch})) {$orderby})";
			}else
			{
				$query = "INSERT INTO {$temptable} (
					SELECT 
					{$posts}.ID, 0 AS type, 0 AS post_parent,0 as useit 
					FROM {$posts}{$pricequery}{$salequery}{$skuquery}{$custommetasearch}
					WHERE ({$posts}.post_type='product'{$titlelike}{$desclike}{$shortdesclike} {$wherenotin} AND {$posts}.post_status IN ({$posttypesearch}){$idquery} ) {$orderby} {$limitquery}) ";
			}
		}
...
//				$id->post_content = str_replace(chr(160)," ", $id->post_content);
	...
	if($arrduplicate === null)
	{
		$query ="SELECT p1.ID, p1.post_title,p1.post_parent, {$meta}.meta_key, {$meta}.meta_value
			...
			AND ({$meta}.meta_key IN ('_regular_price','_sale_price','_sku','_weight','_length','_width','_height','_stock','_stock_status','_visibility','_virtual','_download_type','_download_limit','_download_expiry')) WHERE p1.ID IN (SELECT ID FROM {$temptable}{$useit})";
...
//		$metavals = $arrduplicate;
		...
		$query ="SELECT p1.ID, p1.post_title,p1.post_parent, {$meta}.meta_key, {$meta}.meta_value
			FROM {$posts} p1
			INNER JOIN {$meta} ON p1.ID={$meta}.post_id 
			AND ({$meta}.meta_key IN ('_regular_price','_sale_price','_sku','_weight','_length','_width','_height','_stock','_stock_status','_visibility','_virtual','_download_type','_download_limit','_download_expiry'))	WHERE p1.ID IN ({$duplicateids})";
		$metavals =  $wpdb->get_results($query);
	...
	if($arrduplicate === null)
	{
		$query ="SELECT p1.ID, p1.post_title,p1.post_parent, {$meta}.meta_key, {$meta}.meta_value
			FROM {$posts} p1
			INNER JOIN {$meta} ON p1.ID={$meta}.post_id 
			AND ({$meta}.meta_key IN ('_downloadable_files','_downloadable','_sale_price_dates_from','_sale_price_dates_to','_tax_class','_tax_status','_backorders','_manage_stock','_featured','_purchase_note')) WHERE p1.ID IN (SELECT ID FROM {$temptable}{$useit})";
		$metavals =  $wpdb->get_results($query);
		if ( is_wp_error($metavals) ) {
			return new WP_Error( 'db_query_error', 
				__( 'Could not execute query' ), $wpdb->last_error );
	...
	}else
	{
		...
		$query ="SELECT p1.ID, p1.post_title,p1.post_parent, {$meta}.meta_key, {$meta}.meta_value
			FROM {$posts} p1
			INNER JOIN {$meta} ON p1.ID={$meta}.post_id 
			AND ({$meta}.meta_key IN ('_downloadable_files','_downloadable','_sale_price_dates_from','_sale_price_dates_to','_tax_class','_tax_status','_backorders','_manage_stock','_featured','_purchase_note'))	WHERE p1.ID IN ({$duplicateids})";
		$metavals =  $wpdb->get_results($query);
...
//second query to reduce ram usage
		...
		if($arrduplicate === null)
	{
		$query ="SELECT p1.ID, p1.post_title,p1.post_parent, {$meta}.meta_key, {$meta}.meta_value
			FROM {$posts} p1
			INNER JOIN {$meta} ON p1.ID={$meta}.post_id 
			AND ({$meta}.meta_key IN ('_variation_description','_sold_individually','_product_url','_button_text','_thumbnail_id','_product_image_gallery','_upsell_ids','_crosssell_ids','_product_attributes','_default_attributes'{$customfields})
...
//					do_action( 'delete_post',$ID);
	...
	public static function GetAttributes(&$attributes,&$attrmapslugtoname,$ball = false,$attrname = '')
	{
		$curr_settings = get_option('w3exabe_settings');
		if(is_array($curr_settings))
		{
